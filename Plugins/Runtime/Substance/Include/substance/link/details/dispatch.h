...
#ifndef _SUBSTANCE_LINK_DISPATCH_H
#define _SUBSTANCE_LINK_DISPATCH_H
 ...
 * function. */
unsigned int link_init_dispatch_subsystem(unsigned int signal_main);
...
unsigned int link_shutdown_dispatch_subsystem(void);

/* Tells the dispatch threads to consume any processes they need to */
unsigned int link_flag_dispatch(void);

/* Await a signal from the dispatch threads. */
unsigned int link_await_dispatch(void);

#if defined(__cplusplus)
}
#endif /* __cplusplus */

#endif /* _SUBSTANCE_LINK_DISPATCH_H */
