...
#ifndef _SUBSTANCE_LINK_DETAILS_STRING_UTILS_H
#define _SUBSTANCE_LINK_DETAILS_STRING_UTILS_H
...
void* link_memrchr(const void *buffer, int c, size_t length);
...
char* link_strndup(const char *s, size_t n);
...
size_t link_chomp(char *s, char delim);

/* Performs a search for the length of the string, checking whether there is
 * a null terminator in the buffer. */
unsigned int link_untrusted_strlen(const char *buffer, size_t buffer_length,
                                   size_t *length);
...
#endif /* _SUBSTANCE_LINK_DETAILS_STRING_UTILS_H */
