...
#ifndef _SUBSTANCE_LINK_AVAILABLE_QUEUE_H
#define _SUBSTANCE_LINK_AVAILABLE_QUEUE_H
...
unsigned int link_available_queue_init(void);
...
unsigned int link_available_queue_shutdown(void);

/* Appends the context to the available queue. This returns a success code
 * code on a successful push. */
unsigned int link_available_queue_push(unsigned int context);
...
unsigned int link_available_queue_pop(unsigned int *context);

#if defined(__cplusplus)
}
#endif /* __cplusplus */

#endif /* _SUBSTANCE_LINK_AVAILABLE_QUEUE_H */
